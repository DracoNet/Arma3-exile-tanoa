...
BS_count_shipwrecks = 5; // Total wrecks

BS_locations_crateWreckOffset = 10; // Distance from wreck to spawn crate.
BS_locations_center = [7738.8,7554.9,0]; // Center o fmap from which to spawn wrecks, on Altis this is in the central bay
BS_locations_distance_min = 0; // Minimum distance from BS_location_center to spawn crate.
BS_locations_distance_max = 7500; // Maximum distance from BS_locations_center to spawn crate. Careful putting this too high or they will spawn off the map!
...
[   // [class (if array, picks one random item), guaranteed amount, possible random additional amount, % chance of spawning additional random amount]
    ["Exile_Item_SafeKit", 0, 1, 50], // 100% of the time 0-1 safes will spawn.
    ...
    ["SatchelCharge_Remote_Mag", 1, 2, 20],
